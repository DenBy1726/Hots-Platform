...
{
    public class SpecialistRaiting : AbstractStatItem<HeroStatisticItemAvg[]>
    {
        private Func<string,int> heroGameCount;

        public const string name = "Рейтинг специалиста";
        public override string Name => name;

        public SpecialistRaiting(Func<int,Hero> hero,Func<string,Image> image, 
        ...
        {
            var val = points.Select((x, i) => new Tuple<double, int>(x.specialistRating, x.heroId)).ToList();
            val.Sort();
        ...
        public override void Draw(ZedGraphControl graph, HeroStatisticItemAvg[] points, Color c)
        {
            ...
            graph.GraphPane.Title.Text = "Рейтинг специалиста";
            graph.GraphPane.YAxis.Title.Text = "Рейтинг специалиста";
