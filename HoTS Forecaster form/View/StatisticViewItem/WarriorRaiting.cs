...
{
    public class WarriorRaitingStat : AbstractStatItem<HeroStatisticItemAvg[]>
    {
        private Func<string,int> heroGameCount;

        public const string name = "Рейтинг бойца";
        public override string Name => name;

        public WarriorRaitingStat(Func<int,Hero> hero,Func<string,Image> image, 
        ...
        {
            var val = points.Select((x, i) => new Tuple<double, int>(x.warriorRating, x.heroId)).ToList();
            val.Sort();
        ...
        public override void Draw(ZedGraphControl graph, HeroStatisticItemAvg[] points, Color c)
        {
            ...
            graph.GraphPane.Title.Text = "Рейтинг бойца";
            graph.GraphPane.YAxis.Title.Text = "Рейтинг бойца";
