...
{
    public class SupportRaitingStat : AbstractStatItem<HeroStatisticItemAvg[]>
    {
        private Func<string,int> heroGameCount;

        public const string name = "Рейтинг поддержки";
        public override string Name => name;

        public SupportRaitingStat(Func<int,Hero> hero,Func<string,Image> image, 
        ...
        {
            var val = points.Select((x, i) => new Tuple<double, int>(x.supportRating, x.heroId)).ToList();
            val.Sort();
        ...
        public override void Draw(ZedGraphControl graph, HeroStatisticItemAvg[] points, Color c)
        {
            ...
            graph.GraphPane.Title.Text = "Рейтинг поддержки";
            graph.GraphPane.YAxis.Title.Text = "Рейтинг поддержки";
