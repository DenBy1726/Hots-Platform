...
{
    public class AssasinRaitingStat : AbstractStatItem<HeroStatisticItemAvg[]>
    {
        private Func<string,int> heroGameCount;
        private const string name = "Рейтинг убийцы";
        public override string Name => name;

        public AssasinRaitingStat(Func<int,Hero> hero,Func<string,Image> image, 
        ...
        {
            var val = points.Select((x, i) => new Tuple<double, int>(x.assassinRating, x.heroId)).ToList();
            val.Sort();
        ...
        public override void Draw(ZedGraphControl graph, HeroStatisticItemAvg[] points, Color c)
        {
            ...
            graph.GraphPane.Title.Text = "Рейтинг убийцы";
            graph.GraphPane.YAxis.Title.Text = "Рейтинг убийцы";
