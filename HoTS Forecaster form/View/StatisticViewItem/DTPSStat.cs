...
{
    public class DTPSStat : AbstractStatItem<HeroStatisticItemAvg[]>
    {
        private Func<string,int> heroGameCount;

        public const string name = "Количества полученного урона";
        public override string Name => name;

        public DTPSStat(Func<int,Hero> hero,Func<string,Image> image, 
        ...
        {
            var val = points.Select((x, i) => new Tuple<double, int>(x.damageTakenPerSec, x.heroId)).ToList();
            val.Sort();
        ...
        public override void Draw(ZedGraphControl graph, HeroStatisticItemAvg[] points, Color c)
        {
            ...
            graph.GraphPane.Title.Text = "График количества полученного урона в секунду";
            graph.GraphPane.YAxis.Title.Text = "Количества полученного урона";
