...
{
    public class EXPStat : AbstractStatItem<HeroStatisticItemAvg[]>
    {
        private Func<string,int> heroGameCount;

        public const string name = "Количество опыта";
        public override string Name => name;

        public EXPStat(Func<int,Hero> hero,Func<string,Image> image, 
        ...
        {
            var val = points.Select((x, i) => new Tuple<double, int>(x.expPerSec*x.sec, x.heroId)).ToList();
            val.Sort();
        ...
        public override void Draw(ZedGraphControl graph, HeroStatisticItemAvg[] points, Color c)
        {
            ...
            graph.GraphPane.Title.Text = "График добытого опыта для команды";
            graph.GraphPane.YAxis.Title.Text = "Количество опыта";
