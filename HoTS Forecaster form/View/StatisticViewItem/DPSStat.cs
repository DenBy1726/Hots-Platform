...
{
    public class DPSStat : AbstractStatItem<HeroStatisticItemAvg[]>
    {
        private Func<string,int> heroGameCount;

        public const string name = "Количества нанесенного урона";
        public override string Name => name;

        public DPSStat(Func<int,Hero> hero,Func<string,Image> image, 
        ...
        {
            var val = points.Select((x, i) => new Tuple<double, int>(x.dps, x.heroId)).ToList();
            val.Sort();
        ...
        public override void Draw(ZedGraphControl graph, HeroStatisticItemAvg[] points, Color c)
        {
            ...
            graph.GraphPane.Title.Text = "График количества нанесенного урона героям в секунду";
            graph.GraphPane.YAxis.Title.Text = "Количества нанесенного урона";
